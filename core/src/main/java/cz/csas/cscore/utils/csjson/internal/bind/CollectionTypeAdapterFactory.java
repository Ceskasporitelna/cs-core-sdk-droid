...
public final class CollectionTypeAdapterFactory implements TypeAdapterFactory {
  ...
  public <T> TypeAdapter<T> create(CsJson csJson, TypeToken<T> typeToken) {
    ...
    Type elementType = $CsJson$Types.getCollectionElementType(type, rawType);
    TypeAdapter<?> elementTypeAdapter = csJson.getAdapter(TypeToken.get(elementType));
    ObjectConstructor<T> constructor = constructorConstructor.get(typeToken);

    @SuppressWarnings({"unchecked", "rawtypes"}) // create() doesn't define a type parameter
    TypeAdapter<T> result = new Adapter(csJson, elementType, elementTypeAdapter, constructor);
    return result;
  }
  ...
  private static final class Adapter<E> extends TypeAdapter<Collection<E>> {
    ...
    public Adapter(CsJson context, Type elementType,
