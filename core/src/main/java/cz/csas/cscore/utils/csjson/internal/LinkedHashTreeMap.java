...
import cs.cz.cscore.BuildConfig;
...
public final class LinkedHashTreeMap<K, V> extends AbstractMap<K, V> implements Serializable {
  ...
  private void replaceInParent(Node<K, V> node, Node<K, V> replacement) {
    ...
    if (parent != null) {
      if (parent.left == node) {
        ...
        if (BuildConfig.DEBUG && parent.right != node)
      ...
      if (delta == -2) {
        ...
        if (rightDelta == -1 || (rightDelta == 0 && !insert)) {
          rotateLeft(node); // AVL right right
            ...
            if (BuildConfig.DEBUG && rightDelta != 1)
                throw new AssertionError();
        ...
        if (leftDelta == 1 || (leftDelta == 0 && !insert)) {
          rotateRight(node); // AVL left left
            ...
            if (BuildConfig.DEBUG && leftDelta != 1)
                throw new AssertionError();
        ...
        if (insert) {
          ...
          if (BuildConfig.DEBUG && delta != 1 && delta != 1)
              throw new AssertionError();
   ...
   * it as a LinkedHashMap so that they won't need CsJson on the other side to
