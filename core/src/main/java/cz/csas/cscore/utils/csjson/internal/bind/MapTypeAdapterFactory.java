...
public final class MapTypeAdapterFactory implements TypeAdapterFactory {
  ...
  public <T> TypeAdapter<T> create(CsJson csJson, TypeToken<T> typeToken) {
    ...
    Class<?> rawTypeOfSrc = $CsJson$Types.getRawType(type);
    Type[] keyAndValueTypes = $CsJson$Types.getMapKeyAndValueTypes(type, rawTypeOfSrc);
    TypeAdapter<?> keyAdapter = getKeyAdapter(csJson, keyAndValueTypes[0]);
    TypeAdapter<?> valueAdapter = csJson.getAdapter(TypeToken.get(keyAndValueTypes[1]));
    ObjectConstructor<T> constructor = constructorConstructor.get(typeToken);

    @SuppressWarnings({"unchecked", "rawtypes"})
    // we don't define a type parameter for the key or value types
    TypeAdapter<T> result = new Adapter(csJson, keyAndValueTypes[0], keyAdapter,
   ...
   */
  private TypeAdapter<?> getKeyAdapter(CsJson context, Type keyType) {
    return (keyType == boolean.class || keyType == Boolean.class)
        ? cz.csas.cscore.utils.csjson.internal.bind.TypeAdapters.BOOLEAN_AS_STRING
  ...
  private final class Adapter<K, V> extends TypeAdapter<Map<K, V>> {
    ...
    public Adapter(CsJson context, Type keyType, TypeAdapter<K> keyTypeAdapter,
