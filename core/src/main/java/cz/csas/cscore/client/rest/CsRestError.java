...
public class CsRestError extends CsSDKError {
  public static CsRestError networkError(String url, IOException exception) {
    return new CsRestError(exception.getMessage(), url, null, null, null, Kind.NETWORK,
        exception);
  }

  public static CsRestError conversionError(String url, Response response, Converter converter,
      Type successType, ConversionException exception) {
    return new CsRestError(exception.getMessage(), url, response, converter, successType,
        Kind.CONVERSION, exception);
  }

  public static CsRestError httpError(String url, Response response, Converter converter,
      Type successType) {
    String message = response.getStatus() + " " + response.getReason();
    return new CsRestError(message, url, response, converter, successType, Kind.HTTP, null);
  }

  public static CsRestError unexpectedError(String url, Throwable exception) {
    return new CsRestError(exception.getMessage(), url, null, null, null, Kind.UNEXPECTED,
        exception);
  }
  ...
  CsRestError(String message, String url, Response response, Converter converter,
