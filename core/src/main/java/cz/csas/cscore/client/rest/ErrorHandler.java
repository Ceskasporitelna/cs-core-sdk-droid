...
/**
 * A hook allowing clients to customize {@link cz.csas.cscore.client.rest.client.Response response} exceptions.
 *
 * @author Sam Beran sberan@gmail.com
 */
public interface ErrorHandler {
  /**
   * Return a custom exception to be thrown for a {@link CsRestError}. It is recommended that you
   ...
   *   &#64;Override public Throwable handleError(CsCoreRestError cause) {
   ...
   * </pre>
   *
   * @param cause the original {@link CsRestError} exception
   ...
   */
  Throwable handleError(CsRestError cause);

  /** An {@link ErrorHandler} which returns the original error. */
  ErrorHandler DEFAULT = new ErrorHandler() {
    @Override public Throwable handleError(CsRestError cause) {
      return cause;
    }
  };
}
